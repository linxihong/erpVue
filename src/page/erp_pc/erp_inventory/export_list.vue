    ...
    <div class="common-wrap">
        <div class="common-left" :style="{width: (100 - otherVariable.contentWidth) + '%'}" v-show="!otherVariable.isSpread">
            ...
            <div class="common-btn-wrap">
                <el-button class="btn-cyan add-btn" @click="handleMulAudit" :disabled="otherVariable.isSaving">批量核算</el-button>
        ...
        <div class="common-right" :style="{width: otherVariable.contentWidth + '%'}">
            ...
            >
                ...
                <template slot="main-btn-wrap">
                    <!--<div  class="xs-audit-icon" @click="handleAudit" >核算</div>-->
                    <!--v-if="showUnAudit"-->
                </template>
                <!--头部标题-->
    ...
    export default {
        ...
        data() {
            return {
                ...
                //详情映射列表
                formProp: [
                    {name:'出库单号',prop:'refDocCode',type:'outter'},
                    {name:'出库日期',prop:'docDate',type:'outter'},
                    {name:'单据状态',prop:'docStatusName',type:'outter'},
                    {name:'客户',prop:'clientCode',type:'inner'},
                    {name:'仓库代码',prop:'clientCode2',type:'inner'},
                    {name:'司磅员',prop:'weighman',type:'inner'},
                    {name:'理论总量',prop:'theoryWght',type:'inner'},
                    {name:'经销商',prop:'clientCode',type:'inner'},
                    {name:'装车单号',prop:'refDocCode',type:'inner'},
                    {name:'仓管员',prop:'stockman',type:'inner'},
                    {name:'实际总重',prop:'actWght',type:'inner'},
                ...
                //订单明细映射表
                tabPropI: [
                    {prop:'', name:'',type:'TableIndex',isShort: true,width:'40',align:'center'},
                    {
                        prop:'',name:'型材',
                        children: [
                            {prop:'matCode',name:'型号',width: 55, align:'center',type:'normal'},
                            {prop:'matName',name:'名称',width: 55, align:'center',type:'normal'}
                        ]
                    },
                    {
                        prop:'',name:'颜色',
                        children: [
                            {prop:'cx1',name:'代码',width: 55, align:'center',type:'normal'},
                            {prop:'cx1Name',name:'名称',width: 55, align:'center',type:'normal'}
                        ]
                    },
                    {prop:'cx2Name',name:'材质',type:'normal'},
                    {prop:'cx4Name',name:'长度',type:'normal',rule:'limitLength',unit:' m',isShort: true,align:'right'},
                    {prop:'cx3',name:'膜厚',type:'normal',unit:'',isShort: true,align:'right'},
                    ...
                    {prop:'unitWght',name:'支重',type:'normal',unit:' kg/pcs',isShort: true,align:'right'},
                    {prop:'theroyWght',name:'理重总重',type:'normal',rule:'limitLength',unit:' kg',isShort: true,align:'right'},
                    {prop:'actualWght',name:'实际总量',type:'normal',rule:'limitLength',unit:' kg',isShort: true,align:'right'},
                    {prop:'unitSqm', name:'单支方数',type:'normal'},
        ...
        methods: {
            ...
            getData() {
                this.tableData = [];
                this.fetch(`/oms/wms/WmsStockoutMasterController/getStockoutMasterList?pageNo=${this.pageObj.commonPage}&pageSize=${this.pageObj.commonSize}`,this.searchForm)
                    .then(result => {
            ...
            handleDetail(row) {
                ...
                this.detailForm = {};
                this.fetch('/oms/wms/WmsStockoutMasterController/getStockoutMasterList',{fid:row.fid})
                    .then(result => {
                ...
                this.getAjax(`/oms/wms/WmsStockoutDetailController/getStockoutDetailListByMaster?masterId=${row.fid}`)
                    .then(result => {
                        this.loadingI = false;
