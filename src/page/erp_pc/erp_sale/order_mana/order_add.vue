    ...
    <div class="common-wrap">
        <div class="common-left" :style="{width: (100 - otherVariable.contentWidth) + '%'}" v-show="!otherVariable.isSpread">
            ...
            <div class="common-btn-wrap">
                ...
                <el-checkbox style="margin-left: 10px;" v-model="isSelf" @change="changeSelf">自己单</el-checkbox>
            </div>
            <!--搜索区域-->
            <CommonSearch
        ...
        <div class="common-right" :style="{width: otherVariable.contentWidth + '%'}">
            ...
            >
                ...
                <template slot="main-btn-wrap" v-if="detailId&&!otherVariable.isChildEdit">
                    <!--<el-button class="kucun-btn" @click="stockOrder">模拟库存</el-button>-->
                    <!--<el-button class="kucun-btn" @click="copyAdd">复制新增</el-button>-->
                    <div v-if="showButton" class="xs-icon-confirm xs-icon-first" style="right: 270px;" :class="otherVariable.isSaving ? 'banClick' : ''" @click="!otherVariable.isSaving && handleAudit()">审核</div>
                    <div v-if="isBanOperate" class="xs-icon-confirm" style="right: 210px;" :class="otherVariable.isSaving ? 'banClick' : ''" @click="!otherVariable.isSaving && handleConfirm()">提交</div>
                    <div v-if="detailId&&docStatus===50" class="xs-icon-confirm"  style="right: 210px;" :class="otherVariable.isSaving ? 'banClick' : ''" @click="!otherVariable.isSaving && handleCanCelConfirm()">撤回</div>
                    <i v-if="isBanOperate" class="sm-icon-edit el-icon-edit" title="编辑" style="right: 165px;" @click="handleEdit"></i>
                    <i v-if="detailId&&docStatus < 100" class="sm-icon-edit el-icon-news" title="作废" style="right: 125px;" :class="otherVariable.isSaving ? 'banClick' : ''" @click="!otherVariable.isSaving && cancelOrder()"></i>
                    <i v-if="isBanOperate" class="sm-icon-edit el-icon-delete" title="删除" style="right: 85px;" :class="otherVariable.isSaving ? 'banClick' : ''" @click="!otherVariable.isSaving && handleDelete()"></i>
                    <!--<i v-if="detailId" class="icon-copy" title="复制新增" @click="copyAdd" style="right: 40px;"></i>-->
                    <i v-if="detailId" class="icon-print" title="打印" @click="handlePrint" style="right: 40px;"></i>
                </template>
                <!--头部标题-->
                    ...
                    <div class="head-form-content">
                        <template v-for="item in formProp">
                            <div v-if="item.type === 'inner' && !item.isLong">
                                <span>{{ item.name }}：</span>
                                <span v-if="!item.isNum">{{ detailForm[item.prop] }}</span>
                                <span v-else>{{ detailForm[item.prop]==0?'否':(detailForm[item.prop])==1?'是':"" }}</span>
                            </div>
                        </template>
                    </div>
                    ...
                    <el-tabs v-model="activeName" class="common-content-tab order_style">
                        <el-tab-pane label="订单明细" name="first">
                            ...
                            >
                                ...
                                <el-table-column label="胶条规格" prop="usrTxt6Name" align="left" :show-overflow-tooltip="true"></el-table-column>
                                <el-table-column label="型材系列" prop="matSeriesName" align="left" :show-overflow-tooltip="true"></el-table-column>
                                <el-table-column label="型材类别" prop="matGroupAlType1" align="left" :show-overflow-tooltip="true"></el-table-column>
                                ...
                                <el-table-column label="自定义型号" :align="otherVariable.isChildEdit?'center':'left'">
                                    <template slot-scope="scope">
                                        <span v-if="!scope.row.isEditState">{{ scope.row.cltMatCode }}</span>
                                        <el-input
                                            ...
                                            @focus="setFocus(5)"
                                            v-if="scope.row.isEditState"
                                            v-model.trim="scope.row.cltMatCode">
                                        </el-input>
                                    </template>
                                </el-table-column>
                                ...
                                <el-table-column label="壁厚" align="right">
                                    <template slot-scope="scope">
                                        <span class="num-cell-r">{{ scope.row.thick}}</span>
                                    </template>
                                </el-table-column>
                                ...
                                <el-table-column label="产品描述" width="110px" :align="otherVariable.isChildEdit?'center':'left'" :show-overflow-tooltip="!otherVariable.isChildEdit">
                                    <template slot-scope="scope">
                                        <span v-if="!scope.row.isEditState">{{ scope.row.productDescription }}</span>
                                        <el-input
                                            style="width:109px"
                                            @keydown.native.down="goDown(9)"
                                            @keydown.native.up="goUp(9)"
                                            data-index
                                            @keydown.enter.native="nextFocus"
                                            @focus="setFocus(11)"
                                            v-if="scope.row.isEditState"
                                            v-model.trim="scope.row.productDescription">
                                        </el-input>
                                    </template>
                                </el-table-column>
                                <el-table-column label="备注" width="200" :align="otherVariable.isChildEdit?'center':'left'" :show-overflow-tooltip="true">
                                    <template slot-scope="scope">
                                        ...
                                        <el-input
                                            style="width:199px"
                                            @keydown.native.down="goDown(10)"
                                            @keydown.native.up="goUp(10)"
                                            data-index
            ...
            <transition name="popup">
                ...
                >
                    ...
                    <template slot="custom-add-form">
                        ...
                        <el-form ref="addOrEditForm" :rules="mainRules" :model="addOrEditForm" :inline="otherVariable.isInlineForm" label-width="110px">
                            ...
                            <el-form-item label="经销商" prop="clientCode2">
                                <SelectEnter
                                    ...
                                    @selected="nextFocus"
                                    ref="clientSelect2"
                                    :inputValue="valueObj.clientCode2"
                                    :selectTHs="valueObj.distributorTHs"
                                    :selectTds="valueObj.distributors"
                                    placeholder="输入经销商代码回车搜索"
                                    @getList="getDistributor"
                                    @setObj="setDistributor"
                                    selectLabel="fcode"
                                    selectValue="fcode"
                                    selectKey="fid">
                                </SelectEnter>
                            ...
                            <el-form-item label="合同编码" prop="contractCode">
                                <el-input
                                    v-model="addOrEditForm.contractCode"
                                    @keydown.enter.native="nextFocus"
                                    data-index
                                    @focus="setFocus(6)"
                                ></el-input>
                            ...
                            <el-form-item label="备注" class="full-remark" prop="remark">
                                <el-input type="textarea" :rows="3" v-model="addOrEditForm.remark"
                                          @keydown.enter.native="nextFocus"
                                          data-index
                                          @focus="setFocus(10)"
                                ></el-input>
                            ...
                            <el-form-item label="生产方式" prop="prdMethodName">
                                <el-select
                                    clearable
                                    v-model="addOrEditForm.prdMethodName"
                                    @change="setPrdMethod"
                                    style="width: 100%"
                                    data-index
                                    @focus="setFocus(16)"
                            ...
                            <el-form-item label="生产基地" prop="supplyCompanyCode">
                                <el-select
                                    clearable
                                    v-model="addOrEditForm.supplyCompanyCode"
                            ...
                            <el-form-item label="客户标签2" prop="digitLowerRate">
                                <el-input
                                    v-model="addOrEditForm.usrLabelText2"
                                    @keydown.enter.native="nextFocus"
                                    data-index
                                    @focus="setFocus(21)"
                                ></el-input>
                        ...
                        <CommonTable
                            v-if="operateType === 'edit' && otherVariable.isSpread"
                            ref="detailTable"
                            :isCenter="true"
                            :isMulTable="true"
                            ...
                            :tableHeight="$store.state.screenHei*0.32"
                        >
                        </CommonTable>
                    </template>
                </CommonAdd>
                    ...
                    <div class="slideMid" style="margin-top: 20px;">
                        ...
                        <div class="tableWrap">
                            ...
                            >
                                ...
                                <el-table-column label="产品描述" width="110px" align="center">
                                    <template slot-scope="scope">
                                        <el-input style="width:109px" v-model.trim="scope.row.productDescription">
                                        </el-input>
                                    </template>
                                </el-table-column>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                tableHeight: this.$store.state.screenHei*0.50,
                mainHeight: 716,
                notAllowUpHeight: 0,
                ...
                searchProp: [
                    ...
                    },
                    {
                        ...
                        prop:'docStatus',
                        type:'selectChange',
                        selectList:[{value:null,label:'全部'},{value:50,label:'已确认'},{value:0,label:'未确认'},{value:100,label:'已审核'},{value:140,label:'已签批'}],
                        isMainSearch:false,
                        isMain:false
                    },
                    {
                ...
                formProp: [
                    ...
                    {name:'合同编码',prop:'contractCode',type:'inner'},
                    {name:'客户PO',prop:'clientPo',type:'inner'},
                    {name:'品牌',prop:'brandName',type:'inner'},
                    {name:'提货方式',prop:'loaderMethodName',type:'inner'},
                    {name:'生产方式',prop:'prdMethodName',type:'inner'},
                    {name:'支数上限(%)',prop:'digitUpperRate',type:'inner'},
                    {name:'支数下限(%)',prop:'digitLowerRate',type:'inner'},
                    {name:'客户标签1',prop:'usrLabelText1',type:'inner'},
                    {name:'客户标签2',prop:'usrLabelText2',type:'inner'},
                    {name:'理重发货',prop:'isTheory',type:'inner',isNum:true},
                    {name:'打捆发货',prop:'isBundle',type:'inner',isNum:true },
                    {name:'锯挂痕',prop:'isSawTrace',type:'inner',isNum:true},
                ...
                tableData: [],
                isSelf: true,
                searchForm: {minStatus:50,minStatusOperator:'<=',followName:this.$store.state.userInfo.empName,isCancel:0,isClose:0},
                addOrEditForm: {},
                addForm: {},
                detailForm: {},
                copyForm: {},
                ...
                detailTableProp: [
                    {prop:'docItem', name:'序号',type:'normal',isShort: true,width:'50'},
        ...
        methods: {
                        ...
                        if(result.data && result.data.list) {
                            ...
                            if(isFirst) {
                                this.$nextTick(() => {
                                    ...
                                    this.topFIds.length ? this.tableData[0].isTop = true : this.tableData[0].isTop = false;
                                })
                            }
                        }else {
                            this.tabDataI = [];
            ...
            changeSelf(val) {
                this.detailId = null;
                this.orderId = null;
                val ? this.searchForm.followName = this.$store.state.userInfo.empName : this.searchForm.followName = null;
                this.getData();
            },
            //取消置顶
            cancelTop() {
                this.searchForm.topFIds = [];
                this.topFIds = [];
                this.handleRefresh();
            },
            //刷新
            ...
            handleReset() {
                this.isSelf = true;
                this.searchForm = {minStatus:50,minStatusOperator:'<=',followName:this.$store.state.userInfo.empName,isCancel:0,isClose:0};
                this.handleRefresh();
            },
            getInitPage() {
            ...
            handleDetail(row) {
                ...
                if(row.docStatus >= 50 && row.docStatus < 100) {
                    this.showButton = true
                }else {
                    this.showButton = false
                }
            ...
            handleAdd() {
                ...
                this.$nextTick(() => {
                    this.otherVariable.isSpread = true;
            ...
            clearSelect() {
                ...
                this.$refs.clientSelect2.inputValueData = '';
            ...
            handleEdit() {
                ...
                this.getBrandList(this.addOrEditForm.clientId);
                if(this.detailForm.fid) {
                    this.isShowStatus.isPopUp = true;
                    this.valueObj.clientParentId = null;
                    //经销商
                    this.$set(this.addOrEditForm,'clientCode2',this.detailForm.clientCode2);
                    this.$set(this.valueObj,'clientCode2',this.detailForm.clientCode2);
                    //客户
                    ...
                    //注胶  injectionFlag
                    this.$set(this.addOrEditForm,'injectionFlag',this.detailForm.injectionPvcBrandName);
                    this.valueObj.injectionFlag = this.detailForm.injectionPvcBrandName;

                    //胶条
                    this.$set(this.addOrEditForm,'stripFlag',this.detailForm.stripPvcBrandName);
                    this.valueObj.stripFlag = this.detailForm.stripPvcBrandName;
                ...
                this.$nextTick(() => {
                    if(this.otherVariable.isSpread) {
                        this.$refs['detailTable'].initStatus();
                    }
                    this.otherVariable.isSpread = true;
                    this.otherVariable.contentWidth = 100;
                })
            ...
            handleSave(type) {
                ...
                this.$refs.addOrEditForm.validate((valid) => {
                    if(valid) {
                        ...
                        let api;
                        if(type === 'add') {
                            api = '/oms/sls/SalesOrderMaster/insert';
                        }else if(type === 'edit'){
                            api = '/oms/sls/SalesOrderMaster/update';
                        }
                        ...
                        this.fetch(api,tempObj)
                            .then(result => {
                                if(result.status === 200) {
                                    ...
                                    this.topFIds.push(result.data);
                                    this.isShowStatus.isPopUp = false;
                                    this.addForm = {};
                                    if(type === 'add') {
                                        this.searchForm.topFIds = this.topFIds;
                                        this.searchForm.isRepeat = 0;
                                        this.handleRefresh()
                                    }else {
                                        this.topFIds = [];
                                        delete this.searchForm.topFIds;
                                        this.getData(false,result.data)
                                    }
                                    this.$nextTick(() => {
                                        this.otherVariable.isSpread = false;
            ...
            //模拟库存
            stockOrder() {
                this.getAjax(`/mock/order-stock/${this.docCode}`)
                    .then(result => {
            ...
            handleConfirm() {
                if(this.detailForm.fid) {
                    this.$confirm('是否提交?', '提示',{
                        ...
                        this.otherVariable.isSaving = true;
                        this.getAjax(`/oms/sls/SalesOrderMaster/confirmOrder?orderId=${this.detailForm.fid}&docStatus=${this.detailForm.docStatus}`)
                            .then(result => {
                                if(result.status === 200) {
                                    this.$message({
                                        message: result.message,
            ...
            handleCanCelConfirm() {
                if(this.detailForm.fid) {
                    this.otherVariable.isSaving = true;
                    this.getAjax(`/oms/sls/SalesOrderMaster/unConfirmOrder?orderId=${this.detailForm.fid}&docStatus=${this.detailForm.docStatus}`)
                        .then(result => {
                            if (result.status === 200) {
                                this.$message({
                                    message: result.message,
            ...
            cancelOrder() {
                if(this.detailForm.fid) {
                    this.$confirm('是否作废此订单?', '提示', {
                        confirmButtonText: '确定',
                        cancelButtonText: '取消',
                        type: 'warning'
                    }).then(() => {
                        this.otherVariable.isSaving = true;
                        this.getAjax(`/oms/sls/SalesOrderMaster/cancelOrder?orderId=${this.detailForm.fid}`)
                            .then(result => {
                                if(result.status === 200) {
                                    this.$message({
                                        message: '作废成功',
            ...
            getChildTable(fid,index,type) {
                ...
                this.getAjax(`/oms/sls/SalesOrderDetail/getOrderDetail?pageSize=1000&orderMasterId=${fid}`)
                    .then(result => {
            ...
            handleDeleteChild(row,index) {
                if(row.fid) {
                    if(confirm('确定删除？') === true) {
                        this.getAjax(`/oms/sls/SalesOrderDetail/delete?fid=${this.tabDataI[index].fid}&variantId=${this.tabDataI[index].variantId}`)
                            .then(result => {
                                if(result.status === 200) {
                                    this.$message({
                                        message: '删除成功',
            ...
            getSummaries(param) {
                ...
                sums[13] = digit;
                //理重
                theoryWght ? sums[16] = theoryWght.toFixed(3) : sums[16] = 0;
                return sums;
            },
            //清空下拉数据
            clearSelectData() {
            ...
            setMatCode(val,isNext = true) {
                if(val) {
                    this.fetch('/oms/BasedataController/getAluModel',{fcode:val})
                        .then(result => {
                            if(result.data && result.data.list && result.data.list.length) {
                                ...
                                //胶条规格
                                this.$set(this.tabDataI[this.editIndex],'usrTxt6Name',item.usrTxt6Name);
                                //壁厚
                                this.$set(this.tabDataI[this.editIndex],'thick',item.thick);
                                //型材类别
                                this.$set(this.tabDataI[this.editIndex],'matGroupAlType1',item.matGroupAlType1);
                                //型材系列
                                this.$set(this.tabDataI[this.editIndex],'matSeriesId',item.matSeriesId);
                                this.$set(this.tabDataI[this.editIndex],'matSeriesCode',item.matSeriesCode);
                                this.$set(this.tabDataI[this.editIndex],'matSeriesName',item.matSeriesName);
                                //撕筋
                                if(!item.isTear) {
                    ...
                    //壁厚
                    this.$set(this.tabDataI[this.editIndex],'thick',null);
                    //型材类别
                    this.$set(this.tabDataI[this.editIndex],'matGroupAlType1',null);
                    //型材系列
                    this.$set(this.tabDataI[this.editIndex],'matSeriesId',null);
                    this.$set(this.tabDataI[this.editIndex],'matSeriesCode',null);
                    this.$set(this.tabDataI[this.editIndex],'matSeriesName',null);
                    //撕筋
            ...
            setColorCode(val,isNext = true) {
                if(val) {
                    ...
                    if(JSON.stringify(item) !== {}) {
                        ...
                        //颜色类别
                        this.$set(this.tabDataI[this.editIndex],'colorGroupName',item.colorGroupName);
                        this.$set(this.tabDataI[this.editIndex],'colorGroupCode',item.colorGroupCode);
                        //喷码
                    ...
                    //颜色类别
                    this.$set(this.tabDataI[this.editIndex],'colorGroupName',null);
                    this.$set(this.tabDataI[this.editIndex],'colorGroupCode',null);
                    //喷码
            ...
            getClient(val) {
                if(this.operateType === 'add') {
                    if(this.valueObj.clientParentId) {
                        this.$refs.commondata.getOrderClient(val);
                    }else {
                        this.$message({
                            message: '请先选择经销商',
                            type: 'warning'
                    ...
                    this.valueObj.clientParentId = this.detailForm.clientId2;
                    this.$refs.commondata.getOrderClient(val);
                }
            },
            setClient(item) {
            ...
            getDistributorByFid(fid) {
                this.$refs.commondata.getDistributorByFid(fid);
            },
            setDistributorByClient(item) {
            ...
            //经销商
            getDistributor(val) {
                this.$refs.commondata.getDistributor(val);
            },
            setDistributor(item) {
                this.clearSelectedClient();
                this.$nextTick(() => {
                    this.$set(this.valueObj,'clientParentId',item.fid);
                    this.$set(this.valueObj,'clientCode2',item.fcode);
                    this.$set(this.addOrEditForm,'clientName2',item.fname);
                    this.$set(this.addOrEditForm,'clientCode2',item.fcode);
                    this.$set(this.addOrEditForm,'clientId2',item.fid);
                })
            ...
            //清空已选客户&品牌
            clearSelectedClient() {
                this.$set(this.valueObj,'clientCode','');
                this.$set(this.addOrEditForm,'clientName','');
                this.$set(this.addOrEditForm,'clientCode','');
                this.$set(this.addOrEditForm,'clientId','');
                this.$set(this.addOrEditForm,'clientType','');
                this.$set(this.addOrEditForm,'clientTypeName','');
                this.$set(this.addOrEditForm,'contractCode','');
                this.$set(this.addOrEditForm,'contractId','');
                this.addOrEditForm.brandName = '';
                this.addOrEditForm.brandId = '';
                this.mainRules.brandName = [];
                this.$refs.clientSelect.inputValueData = '';
            },
                    ...
                    .then(result => {
                        if(result.data && result.data.list) {
                            ...
                            if(this.brandList.length) {
                                this.mainRules.brandName = [{ required: true, message: '请选择品牌', trigger: 'blur' }];
                            }else {
                                this.mainRules.brandName = [];
                            }
                        }else {
