<template>
    <div class="common-wrap">
        ...
        <div class="common-right" :style="{width: otherVariable.contentWidth + '%'}">
            ...
            >
                    ...
                    <div class="header-form" ref="headForm">
                        <template v-for="item in formProp">
                            <div v-if="item.type === 'outter'">
                                ...
                                <span v-if="item.isCheck">{{ detailForm[item.prop] == 0 ? '制单中':(detailForm[item.prop] == 50)? '已提交':(detailForm[item.prop] == 100)?'已审核':'已受理' }}</span>
                            </div>
                        </template>
                    </div>
                </template>
                ...
                <template slot="common-content">
                    ...
                    <div class="tab-btn-wrap" :style="{left: tabLeft}">
                        ...
                        <!--<el-button v-if="isBanOperate" class="btn-cyan" @click="handleDeleteList">批量删除</el-button>-->
                    ...
                    <el-tabs v-model="activeName" class="common-content-tab order_style">
                        <el-tab-pane label="装车明细" name="first">
                            <CommonTable
                                ...
                                :isMulTable="true"
                                mainKey="variantId"
                                @changeCell="changeChildCell"
                                @pureClickRow="pureChildClickRow"
                                :isPureClickRow="isPureClickRow"
                                :isNotHandleSelect="true"
                ...
                >
                    ...
                    <template slot="custom-add-form">
                        <el-form ref="addOrEditForm" :model="addOrEditForm" :rules="mainRules" :inline="otherVariable.isInlineForm" label-width="120px">
                            ...
                            &lt;!&ndash;<div class="inline-form-title">运输信息</div>
            ...
            <!--获取公共数据-->
            <CommonData :valueObj="valueObj" @setDistributorByClient="setDistributorByClient" ref="commondata"></CommonData>
        </div>
        <!--详情表增改弹框-->
        <el-dialog
                    ...
                    <div class="child-table-search">
                        <div class="child-table-form inline-form-add">
                            <el-form :model="searchChildForm" :inline="true" label-width="100px" @submit.native.prevent @keyup.enter.native="searchChildTable">
                                ...
                                <el-form-item label="欠发数">
                                    <el-input v-model="searchChildForm.brandIdList"></el-input>
                                </el-form-item>
                                <el-form-item label=" ">
                        ...
                        <!--<div class="child-table-btn">-->
                            ...
                            <!--<el-button @click="resetChildTable">重置</el-button>-->
                            <!--&lt;!&ndash;<el-button v-show="!isMoreChildSearch" @click="isMoreChildSearch = true">更多查询</el-button>&ndash;&gt;-->
                            <!--&lt;!&ndash;<el-button v-show="isMoreChildSearch" @click="isMoreChildSearch = false">收起</el-button>&ndash;&gt;-->
                        <!--</div>-->
                    </div>
            ...
            return {
                ...
                searchProp: [
                    ...
                    },
                    {
                        ...
                        prop:'docStatus',
                        type:'select',
                        selectList:[{value:null,label:'全部'},{value:0,label:'未提交'},{value:50,label:'已提交'},{value:100,label:'已审核'}],
                        isMainSearch:false,
                        isMain:false
                    },
                    {
                ...
                formProp: [
                    ...
                    {name:'单据状态',prop:'docStatus',type:'outter',isCheck:true},
                ...
                childTablePropII: [
                    ...
                    {prop:'cx4Name',name:'长度',type:'normal',align:'right',isShort:true},
                    {prop:'cx3Name',name:'膜厚',type:'normal',align:'right',isShort:true},
                    {prop:'stockDigit',name:'库存数量',type:'normal',align:'right',isShort:true},
                    {prop:'digit',name:'装车数',type:'normal',align:'right',isCellEdit:true,isShort:true},
                    {prop:'stockTheroyWght',name:'理重',align:'right',rule:'keepLength',type:'normal',isShort:true},
                    {prop:'projectName',name:'工程',type:'normal'},
                ],
        ...
        methods: {
            ...
            handleDeleteList() {
                if(this.selectOrderList.length) {
                    this.$confirm('此操作将批量删除装车明细, 是否继续?', '提示', {
                        confirmButtonText: '确定',
                        cancelButtonText: '取消',
                        type: 'warning'
                    }).then(() => {
                        this.fetch('',this.selectOrderList)
                            .then(result => {
            ...
            handleSave(type) {
                this.$refs.addOrEditForm.validate((valid) => {
                    if(valid) {
                        ...
                        let api;
                        if(type === 'add') {
                            api = '/oms/sls/LoaderDoc/insertMaster';
                        }else {
                            api = '/oms/sls/LoaderDoc/updateMaster';
                        }
                    ...
                    this.$confirm('是否提交?', '提示', {
                        ...
                        this.tabDataI.forEach(item =>{
                            if(item.digit ==0 || item.digit ===null){
                                ...
                                let resultStatus;
                                this.getAjax(`/oms/sls/LoaderDoc/submitLoader?docCode=${this.detailForm.docCode}&loaderFlowId=${this.detailForm.fid}&docStatus=${this.detailForm.docStatus}`)
                                    .then(result => {
            ...
            handleUnConfirm() {
                if(this.detailForm.fid) {
                    let resultStatus;
                    this.getAjax(`/oms/sls/LoaderDoc/unConfirmLoader?docCode=${this.detailForm.docCode}&loaderMasterId=${this.detailForm.fid}&docStatus=${this.detailForm.docStatus}`)
                        .then(result => {
                            resultStatus = result.status;
                            if (result.status === 200) {
                                this.$message({
            ...
            handleDelete() {
                if(this.detailForm.fid) {
                    if(confirm('确定删除？') === true) {
                        this.getAjax(`/oms/sls/LoaderDoc/deleteMaster?fid=${this.detailForm.fid}`)
                            .then(result => {
                                if(result.status === 200) {
                                    this.$message({
                                        message: '删除成功',
            ...
            getChildTable(masterId) {
                ...
                this.loadingI = true;
                this.tabDataI = [];
                this.getAjax(`/oms/sls/LoaderDoc/getDetail?pageSize=1000&loaderMasterId=${masterId}`)
                    .then(result => {
            ...
            // 获取从表的行数据
            pureChildClickRow(row,index) {
                this.changeChildIndex = index;
            },
            // 装车数单元格
            changeChildCell(prop) {
                if(prop === 'digit') {
                    let tempRow = this.tabDataI[this.changeChildIndex];
                    this.tabDataI[this.changeIndex].theoryWght = tempRow.unitWght * tempRow.digit;
                    setTimeout(() => {
                        this.$refs['childTable'].initStatus();
                    },200)
                }
            ...
            getChildTableDataI() {
                ...
                this.searchChildForm.clientId = this.clientId;
                this.searchChildForm.orderId = this.orderId;
                // 获取库存列表
                this.fetch(`/oms/sls/LoaderDoc/getLoaderList?pageNo=${this.childPageI}&pageSize=${this.childSizeI}`,this.searchChildForm)
                    .then(result => {
                ...
                // 获取料框列表
                this.fetch(`/oms/sls/LoaderDoc/getLoaderBoxList?pageNo=${this.childPage}&pageSize=${this.childSize}`,this.searchChildForm)
                    .then(result => {
                        if(result.status === 200 && result.data && result.data.list) {
                            result.data.list.forEach(item => {
                                item.isBox = true
            ...
            confirmDialog() {
                ...
                if(!this.childTableDataII.length) {
                    ...
                    if(!isDigitZero) {
                        this.fetch('/oms/sls/LoaderDoc/insertDetail',chooseList)
                            .then(result => {
                                if(result.status === 200) {
                                    this.$message({
                                        message: '保存成功',
            ...
            handleAddChild(){
                this.canLoadWeight = 0;
                if(this.detailForm.fid) {
                    this.openDialog(this.detailForm.clientId);
                    this.canLoadWeight = this.detailForm.loadWght;
                    this.searchChildForm.clientName = this.detailForm.clientName;
            ...
            handleDeleteChild(row,index) {
                if(row.variantId) {
                    if(confirm('确定删除？') === true) {
                        this.getAjax(`/oms/sls/LoaderDoc/deleteDetail?fid=${this.tabDataI[index].fid}`)
                            .then(result => {
                                if(result.status === 200) {
                                    this.$message({
                                        message: '删除成功',
            ...
            getColorCodeList(val) {
                this.colorCodeList = [];
                if(val) {
                    this.fetch('/oms/BasedataController/getMatColor?pageSize=10000',{fcode:val})
                        .then(result => {
                            if(result.data && result.data.list) {
            ...
            setClient(item) {
                ...
                if(item.parentId) {
                    this.getDistributorByFid(item.parentId);
                }else {
                    this.$set(this.addOrEditForm,'clientName2',null);
                    this.$set(this.addOrEditForm,'clientCode2',null);
                    this.$set(this.addOrEditForm,'clientId2',null);
                }
            },
            ...
            getDeliveryLevel() {
                ...
                this.getAjax('/oms/sys/DataDictionary/getDeliveryLevel?pageSize=100')
                    .then(result => {
                        if(result.data) {
