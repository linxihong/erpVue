    ...
    <div class="common-wrap">
        ...
        <div class="common-right" :style="{width: otherVariable.contentWidth + '%'}">
            ...
            >
                ...
                <template slot="main-btn-wrap" v-if="!otherVariable.isChildEdit">
                    <div class="xs-audit-icon btn-position-I" @click="handlePrice" v-if="showPrice">提交</div>
                    <div class="xs-unaudit-icon btn-position-I" @click="handldUnPrice" v-if="showUnPrice">撤销</div>
                    <div class="xs-icon-confirm btn-position-II menu-icon-finance"></div>
                    <!--<div class="xs-icon-confirm btn-position-III icon-edit" @click="contractVisible = true"></div>-->
                </template>
                <!--头部标题-->
                ...
                <template slot="common-content">
                    ...
                    <div class="tab-btn-wrap" :style="{left: tabLeft}">
                        ...
                        <el-button @click="toggleChildMatcode" v-if="!otherVariable.isChildEdit">{{isShowChildMatCode?'隐藏':'显示'}}隔热子型号</el-button>
                    </div>
                    <!--tab区域-->
                    <el-tabs v-model="activeName" class="common-content-tab order_style">
                        <el-tab-pane label="价格明细" name="first" ref="pricedetail">
                            <!--<div @click="isSaveChildRow" v-if="otherVariable.isChildEdit" class="not-allowed-tableI" :style="{height:editIndex*34+'px',top:'96px'}"></div>-->
                            <!--<div @click="isSaveChildRow" v-if="otherVariable.isChildEdit" class="not-allowed-tableII" :style="{height:$store.state.screenHei*0.60-101-(editIndex+1)*34+'px'}"></div>-->
                            <el-table
                                v-if="isResize"
                                ref="pricetable"
                                tooltip-effect="dark"
        ...
        <el-dialog
            ...
            <el-select
                ...
                filterable
                remote
                placeholder="请输入合同编码进行搜索"
                :remote-method="getContractList"
                v-model.trim="priceContract"
                style="width: 100%"
            >
                <el-option
            ...
            <span slot="footer" class="dialog-footer">
                ...
                <el-button class="btn-cyan add-btn" @click="handleSaveContract">确 定</el-button>
            </span>
        </el-dialog>
        <!--批量修改的弹框-->
        <el-dialog
    ...
    export default {
        ...
        methods: {
            ...
            handleDetail(row) {
                ...
                this.priceContract = row.contractCode;
            ...
            getDetailTable(fid) {
                ...
                this.otherVariable.isChildEdit = false;
                this.isShowNotAllowed = true;
                this.getAjax(`/oms/sls/SalesLoaderPriceController/getLoaderPrice?loaderFlowId=${fid}&contractCode=${this.contractCode}`)
                    .then(result => {
                        this.loadingI = false;
            ...
            getPricePart(fid) {
                this.aluPrice = '';
                this.aluTaxPrice = '';
                this.fetch('/oms/MetalPriceController/getMetalPriceByLoader', {loaderFlowId: fid})
                    .then(result => {
                        if(result.status === 200 && result.data) {
            ...
            handlePrice() {
                this.$confirm('是否提交?','提示',{
                    ...
                    let resultStatus;
                    this.fetch('/oms/sls/SalesLoaderPriceController/submitPrice', {loaderFlowId:this.detailId,priceStatus:50,priceIdsArray:this.$lodash.compact(this.priceIdsArray)})
                        .then(result => {
            ...
            handldUnPrice() {
                this.$confirm('是否撤销?','提示',{
                    ...
                    let resultStatus;
                    this.fetch('/oms/sls/SalesLoaderPriceController/submitPrice', {loaderFlowId:this.detailId,priceStatus:0,priceIdsArray:this.$lodash.compact(this.priceIdsArray)})
                        .then(result => {
                            resultStatus = result.status;
                            if(result.status === 200) {
                                this.$message({
            ...
            handleSaveChild(isMul = false) {
                ...
                let tempObj;
                let api = '/oms/sls/SalesLoaderPriceController/updateLoaderPrice';
                ...
                tempObj = {
                    loaderFlowId: this.detailForm.fid,
            ...
            handleLock(priceId,isLocked) {
                this.fetch('/oms/sls/SalesLoaderPriceController/doLockPrice',{fid:priceId,isLocked,loaderFlowId:this.detailId})
                    .then(result => {
                        if(result.status === 200) {
                            this.$message({
                                message: '锁价成功',
            ...
            toggleChildMatcode() {
                this.isShowChildMatCode = !this.isShowChildMatCode;
                if(!this.isShowChildMatCode) {
                    this.tabDataI = this.$lodash.cloneDeep(this.copyDataI)
                }else {
                    this.tabDataI = this.tabDataI.filter(item => !item.parentVariantId)
                }
            },
            //从表取消操作
            handleCancelChild() {
            ...
            getContractList(val) {
                this.contractList = [];
                if(val) {
                    this.fetch('/oms/pqm/Contract/getContractMaster?pageSize=1000',{docCode:val})
                        .then(result => {
            ...
            // 保存合同
            handleSaveContract() {
                if(this.detailId && this.priceContract) {
                    let resultStatus;
                    this.fetch(`/oms/sls/LoaderDoc/append-contract`, {fId:this.detailId, contractCode: this.priceContract})
                        .then(result => {
                            resultStatus = result.status;
                            if(result.status === 200) {
                                this.$message({
                                ...
                                let msg = (result && result.message) || (result && result.data && result.data.message);
                                this.$message({
                                    message: msg || '保存合同失败',
                                    type: 'error'
                                });
