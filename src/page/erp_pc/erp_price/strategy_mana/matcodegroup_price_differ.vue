            ...
            >
                ...
                <template slot="head-form-content">
                    <div class="head-form-content">
                        <div>
                            <span style="font-size: 27px; font-weight: normal">型号组区域差价</span>
                        </div>
                    </div>
                </template>
    ...
    export default {
        ...
        methods: {
            ...
            handleDetail(row) {
                ...
                this.listId =row;
                this.fetch('oms/pqm/PqmModelGroupRegionController/selectPqmModelGroupRegionNoPage',{modelGroupId:row.fid})
                    .then(result => {
                        if(result.data  && result.data.length) {
            ...
            UpdataorderList(){
                this.childTableData.forEach(item => {
                    item.modelGroupId = this.tableId;
                });
                this.fetch('/oms/pqm/PqmModelGroupRegionController/updatePqmModelGroupRegionList',{pqmModelGroupRegionQueryModelList:this.childTableData})
                    .then(result => {
