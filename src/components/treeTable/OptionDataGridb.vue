...
<template>
    <el-table
        ...
        <el-table-column
                ...
                width="80">
            <template slot-scope="scope">
                <el-checkbox @change="ifFobSelect(ifFob[scope.row.resid],scope.row)" :disabled="scope.row.checked=='true'"
...
//            'dataSource':'setColumnArray',
        ...
        methods: {
                ...
                if (val){
                    this.getAjax('/sys/res/selectPermission/'+ val.resid +"/"+ val.typeid +"/"+ val.authorityid ).then((result) => {
                        ...
                        if (result.status == '200') {
                            ...
                            this.$store.commit('handleChildrenItemPermissions',{data:this.dataSource,type:this.$route.query.resid})
            ...
            setColumnArray(val, oldVal) {
                var _resultArray, _ifFob = {}, _ifAu = {}, _ifFlag = {};
                var _ChildrenItemPermissions = this.$store.getters.ChildrenItemPermissions,
                    _resid = this.$route.query.resid;
                ...
                if(val&&val.length>0){
                    ...
                    var _ChildrenItemPermissions = this.$store.getters.ChildrenItemPermissions,
                            _resid = this.$route.query.resid;
                    //console.log(_ChildrenItemPermissions,_resid)
                    if(_ChildrenItemPermissions[_resid]){
            ...
            setMsg(type,v) {//dataSource
                ...
                _.each(this.ifAu,(v,k)=>{
                    ...
                    //console.log(resultobj[k].resname,resultobj[k].resid,resultobj[k].rightofusing,resultobj[k].authorityflag,resultobj[k].isavailable);
                })
                _.each(this.ifFob,(v,k)=>{
                    resultobj[k]?'':resultobj[k] = _dataSourceObj[k];
                    if(v==true&&_dataSourceObj[k]){
                        //!resultobj[k]?'':resultobj[k] = _dataSourceObj[k];
                        resultobj[k].isavailable = 1;
                    }else{
                        resultobj[k].isavailable = 0;
                    }
                    //console.log(resultobj[k].resname,resultobj[k].resid,resultobj[k].rightofusing,resultobj[k].authorityflag,resultobj[k].isavailable);
                })
                ...
                //console.log(resultobj,this.$store.getters.ChildrenItemPermissions)
            ...
            ifFobSelect(type, v){
                this.ifFob[v.resid] == true?this.ifFob[v.resid] = true:'';
                this.setMsg();
                this.watchSelect(v.resid)
            },
            ifFlagSelect(type, v) {
